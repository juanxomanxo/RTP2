...
/*
    public String[] carreras(){
        return this.inscripciones.getAllKeys();//O(∑|c|)
    }

    public String[] materias(String carrera){
        TrieDictionary materias = (TrieDictionary) this.inscripciones.get(carrera);//O(|c|)
        return materias.getAllKeys();//O(∑|m_c|)
    }
*/ 
...
/* 
    public void cerrarMateria(String materia, String carrera) {
        TrieDictionary nombres = (TrieDictionary) this.inscripciones.get(carrera);//O(|c|)
        Clase salon = (Clase) nombres.get(materia);//O(|m|)
        ...
        for (int i = 0; i < anotados.longitud(); i++) {//O(E_m)
            ...
            this.anotacionesAlumnos.get(alumno);//O(1)
        }
        TrieDictionary nombreCarreras = (TrieDictionary) this.inscripciones.get(carrera);//O(|c|)
        Clase aulaMateria = (Clase) nombreCarreras.get(materia);//O(|m|)
        aulaMateria.cerrar();
    } 
